...
// func to determine microcode state needed (feed IR)
int GetState(int x)
{
    int bits15to11 = (x >> 10) & 0x3E;
    int bit5 = (x >> 5) & 1;

    int total = (bits15to11 | bit5) & 0x3F;
    return total;
}
...
void MEM_stage()
{
    ...
    // PCMUX FOR FETCH SIGNALS
    // Target.PC
    TARGET_PC = PS.MEM_ADDRESS;
    // Trap.PC
    
    // MEM.PCMUX Evaluation
    if (PS.MEM_V)
    {
        ...
        else if (Get_BR_OP(PS.MEM_CS) || Get_UNCOND_OP(PS.MEM_CS))
        {
            MEM_PCMUX = 1;
        }
        else
...
void AGEX_stage()
{
    ...
    int state = GetState(PS.AGEX_IR);
    
    // MEM.ADDRESS evaluation
    if (Get_ADDRESSMUX(PS.AGEX_CS))
    {
        int a, b;
        // ADDR1MUX EVAL A
        a = (Get_ADDR1MUX(PS.AGEX_CS)) ? PS.AGEX_SR1 : PS.AGEX_NPC;
        ...
        case 0:
            ...
            break;
        case 1:
            b = SEXT(PS.AGEX_IR & 0x3F, 6);
            break;
        case 2:
            b = SEXT(PS.AGEX_IR & 0x1FF, 9);
            break;
        case 3:
            b = SEXT(PS.AGEX_IR & 0x7FF, 11);
        }
    ...
    {
        // shf
        memALU = PS.AGEX_SR1 << (PS.AGEX_IR & 0x3F);
    }

    int LD_MEM = !(dep_stall || v_mem_br_stall || mem_stall);
    ...
    if (LD_MEM)
    {
        ...
        // MEM.ADDRESS latching
        NEW_PS.MEM_ADDRESS = memAddr;
        // MEM.NPC
...
void DE_stage()
{
    ...
    int CONTROL_STORE_ADDRESS = GetState(PS.DE_IR); /* You need to implement the logic to
                      set the value of this variable. Look
                      at the figure for DE stage */
    int ii, jj = 0;
    int LD_AGEX; /* You need to write code to compute the value of
            LD.AGEX signal */
    LD_AGEX = !(dep_stall || v_mem_br_stall || mem_stall || v_agex_br_stall);
    ...
    if (LD_AGEX)
    {
        ...
        NEW_PS.AGEX_SR1 = REGS[(PS.DE_IR >> 6) & 0x7];
        // Fix
        NEW_PS.AGEX_SR2 = REGS[PS.DE_IR & 0x7];

        //FIIIXX
        NEW_PS.AGEX_V = PS.DE_V; // not if dependencies
...
void FETCH_stage()
{
    ...
    // ICacheAccess
    int ins;
    icache_access(PC, &ins, &icache_r);
    int state = GetState(ins);

    int LD_PC;
    // Second Logic Block
    int LD_DE = !(dep_stall || v_de_br_stall || v_mem_br_stall || mem_stall || v_agex_br_stall);
    // Assigning PC

    if (LD_DE && icache_r)
    {
        // Generating LD.PC (First Logic Block)
        LD_PC = 1;
        NEW_PS.DE_IR = ins;
        NEW_PS.DE_NPC = PC + 2;
        NEW_PS.DE_V = 1;
...
{
    int store = value;
    int sign = ((store >> (bits - 1)) & 1);
    int mask = 0xFFFF << (bits);
